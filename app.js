...
app.use(express.static('public')); // Use 'public' dir for static files
...
async function createMosaicOrig(gridData, cellType, padding=0) {
...
app.get('/create-mosaic-orig', async (req, res) => {
...
function resolvePath (toCheck, desc = '') {
  const a = toCheck.replace('~', os.homedir);
  const b = path.resolve(a);
  if (!fs.existsSync(b)) throw new Error(`No such ${desc ? '(' + desc + ') ' : ''}directory as ${b}`);
  return b;
}

async function createMosaic(inputDir, outputDir, rows = 5, cols = 5, padding = 5) {
...
app.get('/create-mosaic', async (req, res) => {
  try
  {
    const inputDir = resolvePath(req.query.inputPath,'input');
    const outputDir = resolvePath(req.query.outputPath, 'output');
    const rows = req.query.mosaicRows;
    const cols = req.query.mosaicCols;
    if (isNaN(rows) || isNaN(cols)) throw new Error('Invalid mosaic dimensions requested');

    console.log(`Creating a ${rows} x ${cols} mosaic from images in ${JSON.stringify(inputDir)}...`);

    const padding = 5;
    createMosaic(inputDir, outputDir, rows, cols, padding);
